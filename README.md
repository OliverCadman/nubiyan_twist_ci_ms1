...
At the end of the section are two feature videos of the band playing live, playing a selection of two songs from the latest release. Below the videos is another call-to-action button, inviting the user to watch more videos. This button will take the user to the website's 'Video' page.
...
## Live Shows
The Live Shows page starts with an alert bar spanning the full screen, inviting the user to sign up to the mailing list, to keep up-to-date with the band's latest gig listings. This feature is hidden on mobile screen sizes. Below is another header with background and brand logo. 

Below is a table featuring the band's upcoming gig listings with dates, venues, and a FontAwesome Ticket Icon, linked to the relative online box-offices. The ticket icon changes colour and slightly increases in size upon hovering. An wide-angle image of the band playing to a large audience is featured to the right of the gig listings table, with a caption informing the user of the venue played, and year the image was captured. That particular image was chosen to capture the footfall that the band can bring into venues, so as to reinforce trust in the quality of the band. This image is hidden on mobile screen sizes. Below the image is a spotify player, included to encourage the user to preview the band before buying a ticket, or to simply enjoy the music while they browse upcoming gigs. 
